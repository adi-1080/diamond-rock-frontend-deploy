...
const SectorsLosers = (props) => {
    ...
    //         setError(null);
    //         try {
    //             const apiUrl = `https://diamond-rock-django-backend1.onrender.com/api/screener/day_losers_sectorwise`;
    ...
    //             setLosersData(data.quotes); // Assuming the API returns a 'quotes' array
    //         } catch (err) {
    ...
    useEffect(() => {
        const sortedLosers = [...props.tabData] // Create a copy before sorting
        .sort((a, b) => b["Chg%"] - a["Chg%"]) // Sort descending
        .filter(item => item["Chg%"] < 0); // Remove non-negative Chg%
        setLoosers(sortedLosers); // Update state
        setLoading(false);
    }, [props.tabData]);
    ...
    return (
        ...
        >
            ...
            {/* Table */}
            {!loading && !error && loosers.length > 0 && (
            ...
            {/* No Data State */}
            {!loading && !error && loosers.length === 0 && (
                <div className="text-center text-gray-500">No losers found for this sector.</div>
            )}
        </div>
    );
};
