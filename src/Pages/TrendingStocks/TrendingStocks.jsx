...
const API_MAP = {
  "Price Movers": {
    gainers: "https://diamond-rock-django-backend1.onrender.com/api/screener/day_gainers?percent_change=4&region=in&market_cap=2000000000&price=5&volume=15000&sortField=percentchange&sortAsc=false",
    losers: "https://diamond-rock-django-backend1.onrender.com/api/screener/day_losers?percent_change=4&region=in&market_cap=2000000000&price=5&volume=15000&sortField=percentchange&sortAsc=true"
  },
  "Volume Shockers": {
    gainers: "https://diamond-rock-django-backend1.onrender.com/api/screener/volume_shockers?exchange=BSE&exchange=NSI&volume_ratio=3&sortField=dayvolume&sortAsc=false",
    losers: "https://diamond-rock-django-backend1.onrender.com/api/screener/day_losers?percent_change=4&region=in&market_cap=2000000000&price=5&volume=15000&sortField=percentchange&sortAsc=true"
  },
  "52 Week High Low": {
...
export function TrendingStocks() {
  ...
  useEffect(() => {
    const fetchStocksData = async () => {
      try {
        setLoading(true);
        const apiUrl = API_MAP[activeTab][view];
        const response = await axios.get(apiUrl);
        
        let stocksArray = [];
        
        if (activeTab === "52 Week High Low" && view === "gainers") {
  ...
  const getDisplayData = () => {
    const data = stocksData[activeTab]?.[view];
    if (!data) return [];
    ...
    <SidebarAndNavbar>
      <div className="shadow-lg h-auto p-6 pb-2 bg-gray-100 rounded-t-xl">
        <div className="flex justify-between items-center mb-6">
          <h2 className="font-medium text-2xl">Trending Stocks</h2>
        </div>
      ...
      <div className="shadow-lg h-auto p-6 pb-2 bg-white rounded-b-xl">
        ...
        {loading ? (
          ...
          <div className="overflow-y-auto bg-white rounded-lg p-0">
              ...
              <tbody className="divide-y divide-gray-200">
                {transformStockData(getDisplayData()).map((stock, index) => (
                  <tr 
                    ...
                    onClick={() => navigate(`/stock/${stock.logo}.NS/prices`)} 
            ...
            {!loading && !error && stocksData[activeTab]?.[view]?.fullData?.length > 10 && (
